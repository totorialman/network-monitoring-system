...
type MLConfig struct {
	...
	SigmoidMidpoint     float64
	PortScanScoreMin    float64
	PortScanScoreMax    float64
	DdosScoreMin        float64
	DdosScoreMax        float64
	DdosSeverityMin     int
	DdosSeverityMax     int
}
...
func Load() Config {
	...
	ML: MLConfig{
		ServiceURL:        env("ML_SERVICE_URL", "http://localhost:5000"),
		Timeout:           duration("ML_TIMEOUT", 30*time.Second),
		WindowSizeSeconds: float("ML_WINDOW_SECONDS", 300),
		SigmoidSteepness:  float("ML_SIGMOID_STEEPNESS", 25.0),
		SigmoidMidpoint:   float("ML_SIGMOID_MIDPOINT", 0.22),
		PortScanScoreMin:  float("ML_PORTSCAN_SCORE_MIN", 0.30),
		PortScanScoreMax:  float("ML_PORTSCAN_SCORE_MAX", 0.80),
		DdosScoreMin:      float("ML_DDOS_SCORE_MIN", 0.40),
		DdosScoreMax:      float("ML_DDOS_SCORE_MAX", 0.95),
		DdosSeverityMin:   integer("ML_DDOS_SEVERITY_MIN", 4),
		DdosSeverityMax:   integer("ML_DDOS_SEVERITY_MAX", 5),
	},
		JWT:          JWTConfig{Secret: env("JWT_SECRET", "change-me-minimum-32-characters-secret"), Expiration: duration("JWT_EXPIRATION", 30*24*time.Hour)},
