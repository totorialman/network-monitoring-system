...
func (r *LogRepo) CountSince(ctx context.Context, since time.Time) int64 {
	var n int64
	err := r.db.QueryRowContext(ctx, `SELECT count() FROM network_logs WHERE timestamp > ?`, since.UTC().Format("2006-01-02 15:04:05")).Scan(&n)
	if err != nil {
...
func (r *LogRepo) RawSample(ctx context.Context, agentID string, limit int) []map[string]any {
	...
	log.Printf("INFO: clickhouse RawSample returned %d rows for agent_id=%s", len(out), agentID)
	return out
}